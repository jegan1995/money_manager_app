...
class _PdfReportScreenState extends State<PdfReportScreen> {
  ...
  Future<void> _preview(List<TransactionModel> txns, List<AccountModel> accs) async {
    setState(() => _previewing = true);
    try {
      final name = await _authSvc.getUserName() ??
          _authSvc.currentUser?.email ?? 'User';
      await PdfReportService.previewPdf(
        transactions: txns, accounts: accs,
        month: _selectedMonth, userName: name,
      );
    } catch (e) {
      if (mounted) _snack('Preview failed: $e', Colors.red);
    } finally {
      if (mounted) setState(() => _previewing = false);
    }
  }

  Future<void> _share(List<TransactionModel> txns, List<AccountModel> accs) async {
    ...
    try {
      ...
      if (mounted) _snack('Export failed: $e', Colors.red);
    } finally {
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        IconButton(
                          ...
                          color: _isCurrentMonth ? Colors.grey : const Color(0xFF1565C0),
                        ),
                      ],
                    ),
                  ),
                          ...
                          children: [
                            ...
                            if (hasData)
                              Row(
                                ...
                                children: [
                                  _stat('Income',  _fmt(totalIncome),  Colors.greenAccent),
                                  _stat('Expense', _fmt(totalExp),     Colors.redAccent),
                                  _stat('Savings', _fmt(netSavings),
                                      netSavings >= 0 ? Colors.greenAccent : Colors.orangeAccent),
                                ],
                              )
                            else
                              Center(child: Text(
                      ...
                      if (hasData)
                        Padding(
                          ...
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.spaceAround,
                            children: [
                              _chip('📋 ${monthTxns.length} total',    Colors.blue),
                              _chip('↑ ${income.length} income',       Colors.green),
                              _chip('↓ ${expenses.length} expenses',   Colors.red),
                            ],
                          ),
                        ),
                        ...
                        child: Column(children: [
                          SizedBox(
                            ...
                            child: ElevatedButton.icon(
                              onPressed: hasData && !_previewing && !_generating
                          ...
                          SizedBox(
                            ...
                            child: OutlinedButton.icon(
                              onPressed: hasData && !_generating && !_previewing
                              ...
                              label: Text(_generating ? 'Generating PDF...' : '📤  Share / Download PDF'),
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _infoRow('📊', 'Page 1 — Summary', 'Income, expenses, savings rate & category bars'),
                        _infoRow('🏦', 'Page 1 — Accounts', 'All account balances'),
                        _infoRow('↑', 'Page 2 — Income list', 'Date, category, note & amount'),
                        _infoRow('↓', 'Page 2 — Expense list', 'Date, category, note & amount'),
                        _infoRow('📤', 'Shareable', 'WhatsApp, Gmail, Drive — or download on Web'),
                      ],
                    ),
                  ),
  ...
  Widget _infoRow(String icon, String title, String desc) => Padding(
  ...
  String _fmt(double v) {
    final abs = v.abs();
    if (abs >= 10000000) return '${v < 0 ? '-' : ''}₹${(abs / 10000000).toStringAsFixed(1)}Cr';
    if (abs >= 100000)   return '${v < 0 ? '-' : ''}₹${(abs / 100000).toStringAsFixed(1)}L';
