...
class SavingsPredictionService {
  ...
  static List<PredictionScenario> generateScenarios(
    ...
    final incomeGrowth =
        avgIncome > 0 ? ((incomeSlope / avgIncome) * 100 * 12).clamp(-10, 20) : 3.0;
    final expenseGrowth =
        avgExpense > 0 ? ((expenseSlope / avgExpense) * 100 * 12).clamp(-5, 15) : 3.0;
    final s2 = _buildScenario(
      ...
      baseIncome: avgIncome,
      baseExpense: avgExpense,
      incomeGrowthYearly: incomeGrowth.toDouble(),
      expenseGrowthYearly: expenseGrowth.toDouble(),
      insight:
